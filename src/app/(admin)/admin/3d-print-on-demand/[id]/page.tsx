...
const AdminSingleOrderPage = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/requests/custom-requests/${id}`, {
                credentials: 'include'
            });
    ...
    const handleFieldUpdate = async (orderId: string, updatedData: Partial<Order>) => {
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/requests/custom-requests/${orderId}`, {
            method: 'PATCH',
            credentials: 'include',
    ...
    return (
        <div className="p-6 space-y-8">
            ...
            <div key={order._id} className="border p-4 rounded-md shadow-sm space-y-4">
                ...
                <Form {...form} >
                    <p className='bg-red-500 text-white p-3 w-fit rounded-xl'>THESE FIELDS ARE ONLY FOR CUSTOM REQUESTS </p>
