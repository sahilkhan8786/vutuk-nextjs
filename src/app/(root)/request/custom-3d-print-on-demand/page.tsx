...
import { Custom3DProductRequest, custom3dProductRequestSchema } from '@/schemas/custom3dProductRequestSchema'
...
// Extract the form type from the schema
type FormValues = z.infer<typeof custom3dProductRequestSchema>;
...
export default function Custom3DProductRequestPage() {
    ...
    async function onSubmit(values: FormValues): Promise<void> {
        try {
            ...
            if (values.modelFile) {
                fileUrl = await uploadToBunny(values.modelFile);
            }
    ...
    // Properly typed file change handler
    const handleFileChange = async (
        e: React.ChangeEvent<HTMLInputElement>,
        field: { onChange: (file: File | undefined) => void },
        setValue: UseFormSetValue<Custom3DProductRequest>
    ): Promise<void> => {
        ...
        try {
            // Upload to BunnyCDN
            const url = await uploadToBunny(selectedFile, (percent: number) => {
                setUploadProgress(percent); // update loader
            });

            // Save only the URL in the form
            setValue('modelFileUrl', url);
        } catch (err) {
            console.error(err);
                                    ...
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input
                                                ...
                                                onChange={async (e) => await handleFileChange(e, field, form.setValue)}
                                            />
                                        </FormControl>
                                        ...
                                        {addresses.map((addr) => (
                                            <div
                                                ...
                                                onClick={() => {
                                                    ...
                                                    form.setValue('addressId', addr._id);
                                                }}
                                            >
