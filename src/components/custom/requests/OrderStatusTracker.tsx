        ...
        <div className="flex flex-col gap-4 mb-6">
            ...
            <div className="flex items-center justify-between gap-2 overflow-x-auto">
                {STATUS_STEPS.map((step, index) => {
                  ...
                  ${isCompleted ? 'bg-green-500 text-white' : isCurrent ? 'border-2 border-blue-500 text-blue-500' : 'bg-gray-200 text-gray-500'}
                `}>
                                    {isCompleted ? <CheckCircle size={20} /> : isCurrent ? <Loader2 className="animate-spin" size={18} /> : index + 1}
                                </div>
                            </div>
